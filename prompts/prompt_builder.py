    ...
    def build_system(self, user_text: str | None = None, mood: str | None = None,
        ...
        if present:
            ...
            desc = ["(имя неизвестно — голос незнакомый, спроси, как его зовут)"
                    ...
                    for p in present]
            blocks.append("Сейчас с тобой говорят (голоса распознаны): "
                          + ", ".join(desc)
                          + ". Обращайся к ним по имени, когда это живо звучит. "
        ...
        # префиксе выше — здесь только меняющиеся данные наблюдений)
        if self.vision_summary:
            seen = self.vision_summary()
            if seen:
                blocks.append("Что ты видела на экране за последние минуты:\n" + seen)
